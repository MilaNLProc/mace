...
from app.mace.config import (
    ALPHA, BETA, EM, MAX_ITERATIONS, RESTARTS, THRESHOLD, FILLER
)
...
class Mace(object):
                ...
                for a in self.active_annotations[d]:
                    ...
                    # if controls is not defined
                        ...
                        self.competence_expected_counts[a][0] += strategy_marginal / \
                            instance_marginal
                        self.competence_expected_counts[a][1] += (self.gold_label_marginals[d][annotation] * self.competence[a][1]
                                                                  / (self.competence[a][0] * self.label_preference[a][annotation]
                                                                     + self.competence[a][1])
                                                                  ) / instance_marginal
    ...
    def get_test(self):
        ...
        assert len(gold) == self.num_instances, 'Gold labels and input file have different number of instances ({} vs {})'.format(
